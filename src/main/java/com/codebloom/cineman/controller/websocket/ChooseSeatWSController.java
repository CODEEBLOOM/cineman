...
public class ChooseSeatWSController {
    ...
    public void selectSeat(@Payload TicketMessage message) {
        ...
        TicketSelectionResult selectionResult = ticketService.createSeatSelection(message.getContent());
        TicketResponseMessage ticketMessage = TicketResponseMessage.builder()
                .type(TICKET_CREATED)
                .content(
                        DummyTicket.builder()
                                .id(selectionResult.getTicketId())
                                .seat(selectionResult.getSeat())
                                .price(selectionResult.getPrice())
                                .status(TicketStatus.SELECTED)
                                .build()
                )
                .totalMoney(ticketService.getTotalMoneyOfTickets(selectionResult.getInvoiceId()))
                .userId(message.getUserId())
                .ticketId(selectionResult.getTicketId())
                .build();
